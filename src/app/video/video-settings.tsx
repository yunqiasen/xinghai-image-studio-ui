...
import type {
  ...
  VideoResolution,
  VideoStudioMode,
} from "@/lib/video-tasks/types";

import { MODEL_SELECTOR_CLASS_NAME } from "../studio/layout-constants";

export type { VideoAspectRatio, VideoDuration, VideoModel, VideoMotion, VideoResolution, VideoStudioMode } from "@/lib/video-tasks/types";
...
export function VideoSettings({ mode, value, assets, models, onChange, onFiles, onRemoveAsset }: VideoSettingsProps) {
  const { t } = useLanguage();
  const selectedModel = models.find((item) => item.slug === value.model) || models[0];
  const ratios = selectedModel?.capabilities.aspect_ratios || [value.aspectRatio];
  const durations = selectedModel?.capabilities.durations.map((item) => item.seconds) || [value.duration];
  const resolutions = selectedModel?.capabilities.resolutions || [value.resolution];
  ...
  return (
    <div className="space-y-3" data-settings-kind="video">
      <div>
        ...
        <label className={`${MODEL_SELECTOR_CLASS_NAME} relative ${models.length ? "cursor-pointer" : "cursor-not-allowed opacity-70"}`}>
          <span className="flex min-w-0 items-center gap-2.5"><span className="grid h-8 w-8 place-items-center rounded-[10px] bg-[#d946ef]/16 text-[#f0abfc]"><Clapperboard size={16} /></span><span className="truncate text-sm font-semibold text-white">{selectedModel?.name || t("studio.videoModelsLogin")}</span></span>
          <ChevronDown className="shrink-0 text-white/48" size={16} />
          <select aria-label={t("studio.videoModel")} className="absolute inset-0 cursor-pointer opacity-0" disabled={!models.length} value={selectedModel?.slug || ""} onChange={(event) => onChange("model", event.target.value)}>
            {!models.length ? <option value="">{t("studio.videoModelsLogin")}</option> : null}
      ...
      {mode === "video-image" ? (
        <section className="rounded-[16px] border border-dashed border-[#67e8f9]/28 bg-[#22d3ee]/7 p-3">
          <div className="flex flex-wrap items-center justify-between gap-2">
            ...
            <label className="inline-flex min-h-10 cursor-pointer items-center gap-1.5 rounded-xl bg-white px-3 py-2 text-xs font-semibold text-[#171626] select-none"><UploadCloud size={14} />{t("studio.uploadStartFrame")}<input accept="image/png,image/jpeg,image/webp" className="hidden" type="file" onChange={(event) => onFiles(event.target.files, "image")} /></label>
          </div>
          {source ? (
      ...
      <div>
        ...
        <div className="grid grid-cols-3 gap-1.5">{durations.map((duration) => <ChoiceButton key={duration} active={value.duration === duration} onClick={() => onChange("duration", duration)}>{t("studio.seconds", { count: duration })}</ChoiceButton>)}</div>
      </div>

      <div>
        <ControlTitle title={t("studio.videoResolution")} help={t("studio.videoResolutionHelp")} />
        <div className="grid grid-cols-3 gap-1.5">{resolutions.map((resolution) => <ChoiceButton key={resolution} active={value.resolution === resolution} onClick={() => onChange("resolution", resolution)}>{resolution.toUpperCase()}</ChoiceButton>)}</div>
      </div>
