...
任何页面、交互、API 接入、配置或部署行为变更，必须按固定顺序收尾：

```text
实现 -> 测试全部通过 -> 更新对应文档 -> 文档一致性检查 -> 提交/push/部署 -> Live 验证
```

- 测试失败时禁止声明完成，禁止提交、push 或部署。
- 测试通过后必须更新对应文档；代码已改但文档未更新，任务不算完成。
- API 接入、字段或错误处理变化：更新 `docs/API_INTEGRATION.md` 的接入矩阵和接口说明，并核对后端 `docs/API.md`、`docs/openapi-studio.yaml`。
- 页面、登录、积分、生图参数、作品或用户流程变化：更新 `README.md` 或对应 `docs/` 使用说明。
- Docker、端口、域名、环境变量或反代变化：更新部署说明和工作区根 `AGENTS.md`。
- 文档必须描述最终真实行为和验证方法，不允许只保留计划或旧行为。
- 文档更新后执行 `git diff --check`，并核对路径、端口、命令和接口示例。
- 纯排查且没有产生代码、配置和运行态变化时可不更新文档，但回报必须写明原因。
- 最终回报必须列出测试命令、结果、更新的文档文件和文档一致性检查结果。
