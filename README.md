...
右侧按任务 `count` 显示加载画布和等待时间。成功后完整展示 `task.images[].url`。单图缩放时始终以画布中心定位，缩小不会向下偏移或被底部裁切；缩放工具栏移动到画布顶部中央，画布内边距缩小以扩大图片显示面积。单图和多图点击后均在当前页面打开模糊背景大图预览，点击背景、关闭按钮或按 `Esc` 退出，不再跳转新页面。失败和取消状态会保留后端中文原因，积分变化通过 `GET /api/auth/me` 重新同步。
