...
`/studio` 使用后端同步生图接口 `POST /api/image/generate`。点击生成后，右侧立即显示与生成张数一致的加载画布和等待时间；接口返回后展示全部 `imageUrls`。单图支持缩放、适应画布、拖拽查看和打开原图，多图按双列网格展示并保留所选宽高比。右侧信息栏显示当前模型、输出参数与作品同步状态，页面外围使用克制的极光留白。
