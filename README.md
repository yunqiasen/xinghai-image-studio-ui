...
视频分类使用独立的 `VideoSettings` 和 `VideoPreview`，不再复用图片参数与图片预览。文生视频提供视频模型、画幅、时长、清晰度和运动强度；图生视频额外提供一张起始图片。右侧使用视频播放器画布（默认 16:9）、播放按钮、时间轴、时长与视频输出信息，不显示图片缩放、打开原图、局部编辑、提示词模板或图片数量。两个视频分类分别保存参数、提示词和起始图片，互不串联。点击“生成视频”仍只提示功能即将上线，不调用图片后端接口，也不伪造视频结果。灵感光谱已从创作类型区域移除。
